...
pushd $FLAKE && git pull --rebase --autostash && (git commit -a -m "rebuild - $HOSTNAME" || true) && (nix flake check --impure && git push || echo "nix flake check failed — not pushing" >&2) && popd
